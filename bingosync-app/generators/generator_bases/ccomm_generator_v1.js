/*
This generator is made and maintained by the Celeste Community Discord to adress issues with srl_v5 and srl_v8
It is originally forked from srl_v5, with some changes backported from srl_v8
*/
...
bingoGenerator = function(bingoList, opts) {
    ...
    // Shuffle code stolen from slr v8
    ...
    for (let i = 0; i < size * size; i++) {
        ...
        // Shuffle goals before doing things to remove goal order mattering
        let shuffledGoals = getShuffledGoals(getDifficulty)
        let RNG = Math.floor(shuffledGoals.length * Math.random());
        if (RNG === shuffledGoals.length) {
            RNG--;
        }
        let j = 0,
            synergy = 0,
            ...
            minSynObj = null;
        do {
            currentObj = shuffledGoals[(j + RNG) % shuffledGoals.length];
            ...
            j++;
        } while ((synergy !== 0) && (j < shuffledGoals.length));
